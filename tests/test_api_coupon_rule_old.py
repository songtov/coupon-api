...
class TestCouponRuleAPI:
    ...
    async def test_create_coupon_rule_invalid_company(self, admin_user, admin_token):
        ...
        with patch("app.api.coupon_rule_router.get_db", return_value=mock_db), \
             patch("app.api.coupon_rule_router.get_current_admin", return_value=admin_user):
    ...
    async def test_list_company_rules(self, admin_user, admin_token, sample_company, sample_coupon_rule):
        ...
        mock_cursor.to_list.return_value = [sample_coupon_rule]
        mock_db.coupon_rules.find.return_value = mock_cursor
        
        with patch("app.api.coupon_rule_router.get_db", return_value=mock_db), \
             patch("app.api.coupon_rule_router.get_current_admin", return_value=admin_user):
            
            response = client.get(
                f"/api/coupon-rules/company/{sample_company['_id']}",
                headers={"Authorization": f"Bearer {admin_token}"}
            )
    ...
    async def test_list_company_rules_invalid_company(self, admin_user, admin_token):
        ...
        with patch("app.api.coupon_rule_router.get_db", return_value=mock_db), \
            ...
            response = client.get(
                f"/api/coupon-rules/company/{ObjectId()}",
                headers={"Authorization": f"Bearer {admin_token}"}
            )
    ...
    async def test_get_coupon_rule(self, admin_user, admin_token, sample_company, sample_coupon_rule):
        ...
        with patch("app.api.coupon_rule_router.get_db", return_value=mock_db), \
            ...
            response = client.get(
                f"/api/coupon-rules/{sample_coupon_rule['_id']}",
                headers={"Authorization": f"Bearer {admin_token}"}
            )
